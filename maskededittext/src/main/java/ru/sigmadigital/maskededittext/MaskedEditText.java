package ru.sigmadigital.maskededittext;
...
public class MaskedEditText extends AppCompatEditText implements View.OnTouchListener, View.OnFocusChangeListener {
    ...
    private class MaskedInputFilter implements InputFilter {
            ...
            if (!(source instanceof SpannableStringBuilder)) {
                ...
                for (int i = start; i < end; i++) {
                    ...
                    if (defaultMaskedSymbols[0]) {
                        isUserInput = false;
                        if( MaskedEditText.this.getText().length() >= dstart+1) {
        ...
        private int skipSymbol(int index) {
            ...
            if(MaskedEditText.this.getText().length() > position) {
                setSelection(position);
                return position;
            }else if(MaskedEditText.this.getText().length() > 0){
                return MaskedEditText.this.getText().length() -1;
            }else{
                return 0;
            }
